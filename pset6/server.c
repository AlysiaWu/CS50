...
#include <strings.h>
#include <unistd.h>
#include <ctype.h> // for tolower() in lookup()
...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // count spaces in 'line' (request line)
            int i = 0;
            int space_count = 0;
            while(line[i] != '\0')
            {
            ...
            // if request does not start with '/'
            if (line[4] != '/')
            {
                error(501);
                continue;
            ...
            // pointer to first '/'
            char* slashptr = &line[4];
            
            // pointer to next ' '
            char* spaceptr = strchr(slashptr, ' ');
            
            // cursor pointer
            char* cptr = slashptr;
            
            // if '"' between slashptr and spaceptr, error
            while (cptr != spaceptr)
            {
                if (*cptr == '"')
                {
            ...
            // pointer to 'http-version'
            cptr = spaceptr + 1;
            // new array for comparison
            char http[] = "HTTP/1.1\r\n\0";
            // pointer to that array
            char* httpptr = &http[0];
            // if spaceptr* != httpptr*, error
            if (strcmp(cptr, httpptr) != 0)
            {
                error(505);
                continue;
            }
            ...
            // reset cptr
            cptr = slashptr;
            // if abspath does not contain '.' , error
            bool dotfound = false;
            while (cptr != spaceptr)
            {
                if (*cptr == '.')
            ...
            char* questptr = NULL;
            int qlength = 0;
            // pointer to question mark if exists (otherwise remains NULL)
            while (cptr != spaceptr)
            {
                cptr++;
                if (*cptr == '?')
            ...
            // declare query (here rather than inside conditions above so it can be accessed later)
            char query[qlength];
            ...
            {
                // set pointer to query
                cptr = &query[0];
                // set questptr to start AFTER '?'
                questptr++;
                
                // copy string at questptr to string at cptr
                strncpy(cptr, questptr, qlength);
            ...
            // declare size for path[] 
            char path[plength + strlen(root) + 1];
            // pointer to path
            cptr = &path[0];
            ...
            // copy abs-path to temp[]
            char temp[plength + 1];
            char* tempptr = &temp[0];
            strncpy(tempptr, slashptr, plength);
            temp[plength] = '\0';
            ...
            // copy .ext from dotptr+1 to extension[]
            char extension[elength + 1];
            char* extptr = &extension[0];
            strncpy(extptr, dotptr + 1, elength);
            extension[elength] = '\0';
